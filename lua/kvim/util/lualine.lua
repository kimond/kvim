...
function M.cmp_source(name, icon)
	icon = icon or Kvim.config.icons.kinds[name:sub(1, 1):upper() .. name:sub(2)]
	local started = false
...
function M.pretty_path(opts)
	...
	return function(self)
		...
		path = Kvim.norm(path)
		local root = Kvim.root.get({ normalize = true })
		local cwd = Kvim.root.cwd()
